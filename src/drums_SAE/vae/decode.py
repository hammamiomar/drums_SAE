...
This module provides the core VAE operations used by both demos:
- VAE decode demo (audio → latents → steer → decode)
- Generation demo (text → diffusion → latents → steer → decode)

Key insight: Both demos use the SAME decode path. The only difference
is where the latents come from (pre-encoded vs generated).
"""
...
def load_vae(device: str | None = None):
    ...
    Uses `model.pretransform.model` (the raw autoencoder) rather than
    `model.pretransform` (the wrapper) for direct latent decoding.
...
def decode_latents_to_audio(
    ...
    This function handles the full decode pipeline:
    1. Denormalize latents (reverse the training normalization)
    2. Reshape for VAE (b t c → b c t)
    3. Decode to audio
    4. Convert to mono float32 in [-1, 1]

    Args:
        z_norm: Normalized latents, shape (n_timesteps, 64) or (batch, n_timesteps, 64)
        vae: Loaded VAE model (from load_vae)
...
def load_latent_stats(npz_path: str, device: str | None = None) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    These statistics are computed during latent pre-encoding and used for:
    - Normalizing latents before SAE (training and inference)
    - Denormalizing latents after SAE steering (for VAE decode)

    Args:
        npz_path: Path to latents NPZ file (e.g., "data/latents_v2.npz")
        device: Device to load tensors on

    Returns:
        Tuple of (mean, std) tensors, each shape (64,)
    """
    if device is None:
        device = DEVICE
