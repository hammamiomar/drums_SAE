...
Functions are config-agnostic - they take necessary parameters directly.
"""
...
def create_alpha_envelope(
    ...
    Shows how steering strength varies across timesteps.

    Args:
        mode: One of "uniform", "segment", "envelope", "temporal_features"
        params: Dict of alpha parameters for this mode
        label: Property name for y-axis label
        n_timesteps: Total number of timesteps (v1=32, v2=16)
        attack_end: Timestep where attack phase ends
        body_end: Timestep where body phase ends (tail starts)
    """
    fig, ax = plt.subplots(figsize=(14, 3), facecolor=COLORS["bg"])
    _setup_ax(ax)

    timesteps = np.arange(n_timesteps)
    # Convert to ms (assuming total duration scales with timesteps)
    total_ms = n_timesteps * (1500 / 32)  # Scale relative to 32 timesteps = 1500ms
    time_ms = timesteps * (total_ms / n_timesteps)

    # Compute alpha envelope based on mode
    if mode == "uniform":
        alphas = np.full(n_timesteps, params.get("alpha", 0.0))
    elif mode == "segment":
        alphas = np.zeros(n_timesteps)
        alphas[:attack_end] = params.get("alpha_attack", 0.0)
        alphas[attack_end:body_end] = params.get("alpha_body", 0.0)
        alphas[body_end:] = params.get("alpha_tail", 0.0)
    elif mode == "envelope":
        alphas = np.linspace(
        ...
        alphas = np.zeros(n_timesteps)
        alphas[:attack_end] = params.get("alpha_attack", 0.0)
        alphas[attack_end:] = params.get("alpha_sustain", 0.0)
    else:
        alphas = np.zeros(n_timesteps)
    ...
    # Phase boundaries
    attack_ms = attack_end * (total_ms / n_timesteps)
    body_ms = body_end * (total_ms / n_timesteps)
    ...
    for x, text in [(attack_ms / 2, "ATK"), ((attack_ms + body_ms) / 2, "BODY"), ((body_ms + total_ms) / 2, "TAIL")]:
        ax.text(
            x, y_pos, text,
            ha="center", va="center",
            color=COLORS["text_muted"],
...
def create_feature_correlation_plot(
    ...
    Args:
        correlations: Full correlation array (n_features,)
...
def create_audio_triplet_display(
    audio_original: np.ndarray | None,
    audio_plus: np.ndarray | None,
    audio_minus: np.ndarray | None,
    sample_rate: int = 44100,
    feature_name: str = "",
) -> plt.Figure:
    """
    Create waveform display for original + steered + steered comparison.

    Used by v2 demo for bulk testing.
    """
    audio_original = _ensure_1d(audio_original)
    audio_plus = _ensure_1d(audio_plus)
    audio_minus = _ensure_1d(audio_minus)

    fig, axes = plt.subplots(1, 3, figsize=(14, 2), facecolor=COLORS["bg"])

    titles = ["ORIGINAL", f"+{feature_name}", f"-{feature_name}"]
    audios = [audio_original, audio_plus, audio_minus]
    colors_list = [COLORS["original"], COLORS["accent"], COLORS["accent_dim"]]

    for ax, audio, title, color in zip(axes, audios, titles, colors_list):
        _setup_ax(ax)
        if audio is not None:
            time = np.linspace(0, len(audio) / sample_rate, len(audio))
            ax.plot(time, audio, color=color, linewidth=0.8)
            ax.set_xlim(0, time[-1])
            ax.set_ylim(-1, 1)
        ax.set_title(title, color=COLORS["text"], fontsize=9, fontfamily="monospace", pad=4)
        ax.set_xlabel("TIME (s)", color=COLORS["text_dim"], fontsize=7, fontfamily="monospace")
